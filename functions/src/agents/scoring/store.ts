// functions/src/agents/scoring/store.ts
...
async function getPendingNodeIds(): Promise<Set<string>> {
  ...
  logger.info(`Scoring: ${pendingNodeIds.size} existing pending update proposals found`);
...
export async function storeValidationProposal(
  ...
  if (pending.has(nodeId)) {
    logger.info(`Scoring: skipping ${nodeType} ${nodeId} — pending proposal already exists`);
    return false;
  }
  ...
  await db.collection("graph_proposals").add({
    ...
    created_by: "scoring-agent",
    status: "pending",
    created_at: FieldValue.serverTimestamp(),
  });

  pending.add(nodeId);
  logger.info(`Scoring: stored update proposal for ${nodeType} ${nodeId}`);
  return true;
}
