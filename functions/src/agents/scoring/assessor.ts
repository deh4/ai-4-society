// functions/src/agents/scoring/assessor.ts
...
async function runAssessment(
  ...
  const systemPrompt = `You are a scoring agent for the AI 4 Society Observatory. Your job is to assess whether a ${nodeType} node's attributes still accurately reflect reality given recent evidence.
...
export async function assessNode(
  ...
  try {
    ...
    if (!result.has_changes || result.confidence < CONFIDENCE_THRESHOLD) {
      logger.info(`Scoring: no changes needed for ${nodeType} ${nodeId} (confidence: ${result.confidence})`);
      return { result: null, tokenUsage };
    }
    return { result, tokenUsage };
  } catch (err) {
    logger.error(`Scoring: failed to assess ${nodeType} ${nodeId}:`, err);
    return { result: null, tokenUsage: { input: 0, output: 0 } };
  }
}
