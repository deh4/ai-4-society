...
**Solution: Two-layer approach — pre-rendering + RSS feeds.**

**Layer 1: Pre-rendering for structural pages.**

Use a pre-rendering approach (e.g., `vite-plugin-ssr`, `prerender-spa-plugin`, or a lightweight SSG build step) to generate static HTML for:
- `/` (Landing) — hero content, risk badges with names, latest feed headlines baked in
- `/about` — full mission/methodology content (the most SEO-important page)
- `/observatory/:nodeId` — one static page per risk/solution node (the most GEO-important pages)

Admin and authenticated routes remain client-rendered only.

**Layer 2: RSS/Atom feeds for dynamic content.**

The news feed updates every 6 hours (Feed Curator), but pre-rendering only runs at deploy time. Between deploys, new feed content is invisible to crawlers. Solution: **publish machine-readable feeds.**

| Feed URL | Content | Format |
|----------|---------|--------|
| `/feeds/all.xml` | All feed items (signals + milestones) | RSS 2.0 + Atom |
| `/feeds/risks.xml` | Signals classified as risk | RSS 2.0 |
| `/feeds/solutions.xml` | Signals classified as solution | RSS 2.0 |
| `/feeds/{nodeId}.xml` | Signals related to a specific risk/solution | RSS 2.0 |

- Generated by Feed Curator alongside `feed_items` (same data, different format)
- Deployed as static XML files to Firebase Hosting
- Linked from HTML via `<link rel="alternate" type="application/rss+xml">` tags
- Google indexes RSS feeds natively and uses them for Google News
- LLM crawlers (Perplexity, ChatGPT) can parse RSS/Atom to get fresh content
- Researchers/journalists can subscribe in their RSS readers — low-friction engagement
- Per-node feeds (`/feeds/R01.xml`) let users follow specific risks

This also gives you a **free newsletter substitute** — users who prefer RSS over email get updates without you building email infrastructure.

**Feed rebuild trigger:** Feed Curator writes both `feed_items` (Firestore) and RSS XML files (Cloud Storage → deployed to Hosting) in the same run. Fresh content is available to crawlers within minutes of a Feed Curator run, not just at deploy time.

**Traditional SEO:**

| Technique | Implementation |
|-----------|---------------|
| Meta tags | Dynamic `<title>` and `<meta description>` per page. Node pages get: "AI Risk: [name] — AI 4 Society Observatory" |
| Open Graph / Twitter cards | Per-node social preview cards (title, description, type badge image). Enables rich link previews when shared. |
| Sitemap | Auto-generated `sitemap.xml` listing all public node URLs. Rebuilt by Graph Builder alongside the snapshot. |
| Canonical URLs | `/observatory/R01` is the canonical URL for each risk node |
| Structured data (JSON-LD) | `Article` schema on feed items, `Organization` schema on About page, `Dataset` schema on observatory pages |
...
| Technique | Implementation |
|-----------|---------------|
| **llms.txt** | Serve `/llms.txt` at root — a concise, structured summary of the observatory for LLM crawlers. Contains: what the platform is, the taxonomy of risks/solutions, how data is collected, and links to key pages. Updated by Graph Builder. |
| **llms-full.txt** | Serve `/llms-full.txt` — a more detailed version including all risk/solution summaries, methodology, and source information. This is what LLMs ingest to answer questions about AI risks citing our platform. |
| **Semantic HTML** | Use proper `<article>`, `<section>`, `<h1>`-`<h3>`, `<time>`, `<cite>` tags. LLM crawlers parse HTML structure more reliably than visual layout. |
| **Quotable content** | Each risk/solution has a clear, self-contained summary paragraph that an LLM can extract and cite. Avoid summaries that only make sense in context. |
| **Attribution-friendly** | Include "Source: AI 4 Society Observatory" in structured data and meta tags. Makes it easy for LLMs to attribute. |
| **Factual density** | Node pages should lead with concrete facts (scores, dates, key players) before narrative. LLMs prefer extractable facts over prose. |

**Implementation approach:**
- Pre-rendering runs as a post-build step in CI (after `vite build`, run pre-renderer)
- `llms.txt` and `llms-full.txt` are generated by Graph Builder and deployed as static files
- `sitemap.xml` is generated by Graph Builder and deployed as a static file
- No SSR server needed — just static HTML for public pages + the SPA bundle for dynamic behavior

**Why this matters for your audiences:**
- A journalist searching Perplexity for "what are the biggest AI risks to society" should find AI 4 Society in the results
- A researcher asking ChatGPT "what organizations track AI societal risks" should get cited
- Someone Googling "AI discrimination risk" should land on your R01 observatory page
