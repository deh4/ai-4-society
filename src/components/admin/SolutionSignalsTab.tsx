...
import type { SolutionSignalItem } from "../../types/review";
...
export default function SolutionSignalsTab() {
  const { user } = useAuth();
  const { snapshot } = useGraph();
  const { handleSignalAction, handleBulkSignalAction, updating } = useSignalActions();

  const [items, setItems] = useState<SolutionSignalItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [statusFilter, setStatusFilter] = useState<string>("pending");
  const [selectedItem, setSelectedItem] = useState<SolutionSignalItem | null>(null);
  const [adminNotes, setAdminNotes] = useState("");
  const [bulkSelectedIds, setBulkSelectedIds] = useState<Set<string>>(new Set());

  // Subscribe to solution signals (solution | both)
  useEffect(() => {
    const signalsRef = collection(db, "signals");
    const constraints = [
      where("signal_type", "in", ["solution", "both"]),
    ...
    const unsub = onSnapshot(q, (snap) => {
      const signals: SolutionSignalItem[] = snap.docs.map((d) => {
        const data = d.data();
        return {
          id: d.id,
          type: "solution-signal" as const,
  ...
  const groupedItems = useMemo(() => {
    const groups = new Map<string, SolutionSignalItem[]>();
  ...
  const selectItem = (item: SolutionSignalItem | null) => {
  ...
  const handleAssign = async (uid: string | null) => {
    ...
    if (uid) await assignItem("signals", selectedItem.id, uid, user.uid);
    else await unassignItem("signals", selectedItem.id);
  };
    ...
    <div className="flex flex-col md:flex-row h-[calc(100vh-105px)]">
      ...
      <div className={`${selectedItem ? "hidden md:flex" : "flex"} w-full md:w-96 border-r border-white/10 flex-col`}>
        <div className="flex items-center gap-2 p-3 border-b border-white/10">
          <span className="text-xs font-medium text-white/60">Solution Signals</span>
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {loading ? (
            ...
            groupedItems.map(([dateKey, group]) => {
              ...
              return (
                <SignalDateGroup key={dateKey} date={dateKey} count={group.length}
                  pendingCount={group.filter((i) => i.status === "pending").length}
                  selectedCount={selectedInGroup} totalInGroup={group.length}
                  onSelectAll={() => setBulkSelectedIds((prev) => {
                    const next = new Set(prev); for (const id of groupIds) next.add(id); return next;
                  })}
                  onDeselectAll={() => setBulkSelectedIds((prev) => {
                    const next = new Set(prev); for (const id of groupIds) next.delete(id); return next;
                  })}>
                  {group.map((item) => (
                    <div key={item.id} className="flex items-start gap-2">
                      ...
                      <button onClick={() => selectItem(item)}
                        className={`flex-1 text-left p-3 border-l-4 border-green-500 rounded-r-lg transition-colors ${
                          selectedItem?.id === item.id ? "bg-white/10 ring-1 ring-white/20" : "bg-white/5 hover:bg-white/8"
                        }`}>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-white/50">
                            {item.signalType === "both" ? "Risk + Solution" : "Solution"}
                          </span>
      ...
      <div className={`${selectedItem ? "flex" : "hidden md:flex"} flex-1 flex-col overflow-y-auto`}>
        {!selectedItem ? (
          <div className="flex items-center justify-center h-full text-gray-500 text-sm">Select a signal to review</div>
        ) : (
          <div className="p-4 md:p-6">
            <button onClick={() => setSelectedItem(null)}
              className="mb-4 text-sm text-gray-400 hover:text-white transition-colors md:hidden">&larr; Back to list</button>
            <div className="max-w-2xl">
              <div className="flex items-start justify-between gap-4 mb-2">
                <h2 className="text-xl font-bold">{selectedItem.title}</h2>
                <AssigneeDropdown currentAssignee={selectedItem.assigned_to}
                  allowedRoles={["signal-reviewer", "lead"]} onAssign={handleAssign} />
              </div>
                ...
                <div className="flex gap-6">
                  {selectedItem.severityHint && (
                    <div>
                      ...
                      <div className={`text-sm font-bold ${selectedItem.severityHint === "Critical" ? "text-red-400" : selectedItem.severityHint === "Emerging" ? "text-orange-400" : "text-gray-400"}`}>
                        {selectedItem.severityHint}
                      </div>
                    </div>
                  )}
                    ...
                    {selectedItem.relatedNodeIds.map((nodeId) => {
                      ...
                      return (
                        <span key={nodeId} className={`text-xs px-2.5 py-1 rounded-lg ${
                          node?.type === "risk" ? "bg-red-400/10 text-red-400 border border-red-400/20"
                            : node?.type === "solution" ? "bg-green-400/10 text-green-400 border border-green-400/20"
                            : "bg-white/5 text-gray-400 border border-white/10"
                        }`}>{node ? `${node.id} · ${node.name}` : nodeId}</span>
                      );
                    })}
                  </div>
                </div>
              ...
              {selectedItem.status === "pending" ? (
                <div className="flex gap-3 flex-wrap">
                  <button onClick={() => onAction(selectedItem.id, "approved")} disabled={updating}
                    className="px-4 py-2 rounded bg-green-600 hover:bg-green-500 text-white text-sm font-medium transition-colors disabled:opacity-50">Approve</button>
                  <button onClick={() => onAction(selectedItem.id, "rejected")} disabled={updating}
                    className="px-4 py-2 rounded bg-red-600 hover:bg-red-500 text-white text-sm font-medium transition-colors disabled:opacity-50">Reject</button>
                  <button onClick={() => onAction(selectedItem.id, "edited")} disabled={updating}
                    className="px-4 py-2 rounded bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium transition-colors disabled:opacity-50">Approve (Edited)</button>
                </div>
              ) : (
                <div className="flex items-center gap-3">
                  <span className={`text-sm px-3 py-1 rounded ${STATUS_COLORS[selectedItem.status] ?? ""}`}>{selectedItem.status}</span>
                  <button onClick={() => onAction(selectedItem.id, "pending")} disabled={updating}
                    className="text-xs text-gray-400 hover:text-white transition-colors">Reset to Pending</button>
                </div>
              )}
            </div>
          </div>
